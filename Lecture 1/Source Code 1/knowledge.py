...
class Sentence:
    ...
    def symbols(self):
        """Returns set of all symbols in the logical sentence"""
        return set()
...
class Implication(Sentence):
    ...
    def __eq__(self, other):
        return (isinstance(other, Implication) and self.antecedent == other.antecedent
                and self.consequent == other.consequent)

    def __hash__(self):
        return hash(
            ("implies", hash(self.antecedent), hash(self.consequent))
        )
...
def model_check(knowledge, query):
    ...
    def check_all(knowledge, query, symbols, model):
        ...
        # (The logic below might be a little confusing: we start with a list of symbols. The functions is recursive,
        # and every time it calls itself it pops one symbol from the symbols list and generates models from it. Thus,
        # when the symbols list is empty, we know that we finished generating models with every possible truth
